...
async function agentLoop(messages: OpenAI.Chat.ChatCompletionMessageParam[]): Promise<void> {
    ...
    while (true) {
        ...
        if (choice.finish_reason !== 'tool_calls' || !assistantMessage.tool_calls || assistantMessage.tool_calls.length === 0) return
        ...
        for (const toolCall of assistantMessage.tool_calls) {
            ...
            const toolCallResult = await executeTool(functionName, functionArgs, toolCall.id)
