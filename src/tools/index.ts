...
export async function executeTool(
    ...
    switch (name) {
        ...
        case 'runRead': {
            ...
            if (!parsed.success) return `Error: Invalid arguments for ${name}: ${parsed.error.message}`
            return toolHandler.runRead(parsed.data.path, toolCallId, parsed.data.limit)
        }
        case 'runWrite': {
        ...
        case 'runEdit': {
            ...
            if (!parsed.success) return `Error: Invalid arguments for ${name}: ${parsed.error.message}`
            return toolHandler.runEdit(parsed.data.path, parsed.data.old_text, parsed.data.new_text)
        }
        default:
